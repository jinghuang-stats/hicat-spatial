...
from .model_utils import tensorbatch2im
torch.multiprocessing.set_sharing_strategy('file_system')


def concat_scores256(attns, w_256=None, h_256=None, size=(256, 256)):
    r"""

    """
    if w_256 is None and h_256 is None:
        grid_size = int(round(len(attns) ** 0.5))
        if grid_size * grid_size != len(attns):
            raise ValueError(
                "w_256 and h_256 are required when attention patches do not "
                "form a square grid."
            )
        w_256 = h_256 = grid_size
    elif w_256 is None or h_256 is None:
        raise ValueError("w_256 and h_256 must be provided together.")
...
def get_patch_attention_scores(
    ...
    """Run a 256-level ViT and return the patch image and attention maps."""
    ...
    batch = transform(patch).unsqueeze(0).to(device256, non_blocking=True)
    with torch.no_grad():
        attention = model256.get_last_selfattention(batch)
        num_heads = attention.shape[1]
        attention = attention[:, :, 0, 1:].reshape(1, num_heads, 16, 16)
...
def create_hierarchical_heatmaps_indiv(region, model256, model4k, output_dir, fname,
                             ...
                             device256=torch.device('cuda:0'), device4k=torch.device('cuda:0')):
    r"""
