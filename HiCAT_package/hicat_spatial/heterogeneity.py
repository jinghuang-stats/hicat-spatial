...
def perm_adjusted_silhouette(
    ...
    if not isinstance(n_perm, (int, np.integer)) or n_perm < 2:
        raise ValueError("n_perm must be an integer greater than or equal to 2.")
    ...
    if one_sided:
        ...
        p = min(1.0, 2 * min(p_upper, p_lower))
...
def compute_final_heterogeneity_score(
    ...
    if scale_score:
        valid_scores = hetero_summary["hetero_score"].dropna()

        if valid_scores.empty:
            hetero_summary["hetero_score_sca"] = np.nan
        elif valid_scores.max() > valid_scores.min():
            ...
            hetero_summary["hetero_score_sca"] = np.where(
                hetero_summary["hetero_score"].notna(),
                0.0,
                np.nan,
            )
