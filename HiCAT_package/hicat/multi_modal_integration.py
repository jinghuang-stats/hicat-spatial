...
def align_modalities_by_obs_names(
    ...
    aligned_adata_dic : dict
        Dictionary of aligned AnnData copies for selected modalities.
    """
    ...
    if not align_by_obs_names:
        ...
        return {m: modality_adata_dic[m].copy() for m in selected_modalities}
    ...
    aligned_adata_dic = {
        modality: modality_adata_dic[modality][common_obs, :].copy()
        for modality in selected_modalities
    }
...
def integrate_modalities_for_adata_dic(
    ...
    base_adata = aligned_adata_dic[selected_modalities[0]].copy()
...
def cluster_integrated_embedding(
    ...
    clustering_config : dict
        ...
        ``n_clusters`` : int
            Number of KMeans clusters. Must be at least 2 and smaller than
            ``n_obs``.
