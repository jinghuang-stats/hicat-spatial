...
class ClusteringConfigStageConfig:
    ...
    ----------
    included_modalities
        Candidate Gene/Image/Protein modalities.
    features_format
        ``"auto"`` uses section-format features for NN selection and
        modality-format features for quantile selection.
    evaluate_all_nodes
        By default only the hierarchy root is evaluated because automatic
        label transfer uses one global clustering configuration. Enable this
        for manual, node-by-node parameter inspection.
    parameters
        Additional parameters forwarded to
        ``determine_multi_modal_embedding_config``. Common keys include
        ``hard_threshold``, ``alpha``, ``selection_criterion``,
        ``pcs_num_dic``, ``candidate_methods``, ``min_spots``, and
