...
::
    ...
    from hicat.pipelines.preprocessing import (
        PreprocessConfig,
...
class PreprocessConfig:
    ...
    reference_sections, query_sections
        Section IDs used in all input filenames.
    modalities
        Available modalities, kept in the canonical Gene/Image/Protein order.
    target_sum, log1p, uppercase_features
        Molecular total normalization, log transformation, and feature-name
    ...
    gene_enhancement, protein_enhancement
    ...
    array_x_key, array_y_key
        Array-grid coordinates used by scan-based contour candidates.
    label_key
        Reference tissue-region annotation column.
    image_template, annotated_image_template
        Input image templates. ``{section}`` is required; ``{ext}`` tries jpg,
        jpeg, png, tif, and tiff automatically.
    color_order
...
class PreprocessPipelineResult:
    ...
    ``annotation_results[section]`` contains reference scribble masks and label
    mappings when color-based annotations were extracted.
    """
    ...
    def get_adata(self, cohort, level, modality, section):
        """Return one AnnData using cohort/level/modality/section keys."""
