...
Each stage can be run independently from ``hicat.pipelines``. This module
...
def run_hicat_workflow(
    ...
    label_transfer_jobs
        Query-specific stage-6 inputs. Their exact dictionary layout follows
        the selected single-reference NN, multi-reference NN, or quantile
        framework. The coordinator injects the hierarchy and modality-specific
        feature results when they are omitted; scaled AnnData dictionaries and
        the final KMeans/Leiden clustering configuration remain explicit.
...
def main():
    ...
    raise SystemExit(
        ...
        "and run_hicat_workflow, or run individual functions from hicat.pipelines. "
